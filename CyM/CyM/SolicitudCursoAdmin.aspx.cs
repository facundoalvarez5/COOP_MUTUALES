...
namespace CyM
{
    public partial class SolictudCursoAdmin : System.Web.UI.Page
    {
        ...
        {
            url = Path.Combine(Request.PhysicalApplicationPath, "Download");
        ...
        {
            get { return (string)Session["archivo"]; }
            set { Session["archivo"] = value; }
        }

        private string url
        {
            get { return (string)Session["url"]; }
            set { Session["url"] = value; }
        }
        private int AsistenteSeleccionado
        {
            get { return (int)Session["AsistenteSeleccionado"]; }
            set { Session["AsistenteSeleccionado"] = value; }
        }
        ...
        {
            IList<SolicitudCursoDto> lista = Bl.GetSolicitudesCurso(txtFiltroNombre.Text, txtFiltroFechaDesde.Text,
        ...
        private void EditarEntidad(EstadoAbmcEnum estado)
        {
            ...
            var SolicitudCurso = Bl.GetSolicitudCursoDtoById(EntidadSeleccionada.ToString());

            
            CargarCamposFormulario(SolicitudCurso);
        ...
        private void HabilitarDehabilitarCampos(bool valor)
        {
            ...
            divGestionAsistentes.Visible = valor;
            FUAsistentes.Enabled = valor;
            btnAdjuntar.Enabled = valor;
            gvAsistentes.Columns[9].Visible = valor;
            txtCantAsistentes.Enabled = valor;
        ...
        private void CargarCamposFormulario(SolicitudCursoDto Solicitud)
        {
            txtNombreCooperativa.Text = Solicitud.NombreCooperativa;
            ddlDepartamento.Items.Clear();
            CargarDepartamentos();
            ddlDepartamento.SelectedValue = Solicitud.LocalidadDto.IdDepartamento;
            ddlLocalidad.Items.Clear();
            CargarLocalidades();
            ddlLocalidad.SelectedValue = Solicitud.LocalidadDto.IdLocalidad;
            txtCantAsistentes.Text = Solicitud.CantidadEstimada.ToString();
            ListaAsistentesCurso = Solicitud.SolicitantesCurso; 
            gvAsistentes.DataSource = Solicitud.SolicitantesCurso;
        ...
        {
            if (btnAdjuntar.Enabled)
            {
                lblMensajeError.Text = "Primero debe adjuntar el archivo de asistentes.";
                divMensajeError.Visible = true;
                return;
            }
            var solicitud = CargarDatosSolicitud();
            solicitud.LinkArchivoAsistentes = url+"\\"+archivo;
            solicitud.NroSolicitudCurso = EntidadSeleccionada.ToString();
            ResultadoRule result = Bl.ActualizarSolicitudCurso(solicitud);
            if (result.OcurrioError)
                {
                    lblMensajeError.Text = "Ocurrió un error en base de datos al actualizar el registro.";
                    divMensajeError.Visible = true;
                ...
                {
                    lblMensajeExito.Text = "Se actualizaron los datos correctamente.";
                    divMensajeExito.Visible = true;
                    divMensajeError.Visible = false;
                    CambiarEstado(EstadoAbmcEnum.CONSULTANDO);
                }
        ...
        {
            var resultado = Bl.EliminarSolicitudCurso(EntidadSeleccionada);
            if (resultado=="OK")
            {
                lblMensajeExito.Text = "La solicitud de curso se eliminó correctamente.";
                divMensajeError.Visible = false;
                divMensajeExito.Visible = true;
                CambiarEstado(EstadoAbmcEnum.CONSULTANDO);
            }
            ...
            {
                lblMensajeError.Text = "Ocurrió un error en base de datos al eliminar la solicitud.";
                divMensajeError.Visible = true;
                divMensajeExito.Visible = false;
            }
        }
        ...
        private SolicitudCurso CargarDatosSolicitud()
        {
            var solicitud = new SolicitudCurso
            {
                ...
                cantAsistentes = txtCantAsistentes.Text,
        ...
        protected void btnBuscarAsistente_OnClick(object sender, EventArgs e)
        {
            ...
            if (solicitante != null)
            {
                ...
                if (!string.IsNullOrEmpty(txtEmail.Text) || !string.IsNullOrEmpty(txtTelefonoFijoCodArea.Text) ||
                    !string.IsNullOrEmpty(txtTelefonoFijo.Text) || !string.IsNullOrEmpty(txtCelularCodArea.Text) ||
                    !string.IsNullOrEmpty(txtCelular.Text))
                {
                    asistente.Contacto = new Comunicacion
                    {
                        ...
                        IdEntidad =
                            solicitante.Sexo.IdSexo + solicitante.NroDocumento + solicitante.PaisTD.Nacionalidad +
                            solicitante.Id_Numero,
        ...
        protected void gvAsistentes_OnRowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (gvAsistentes.SelectedValue != null)
                AsistenteSeleccionado = int.Parse(gvAsistentes.SelectedValue.ToString());

            switch (e.CommandName)
            {
                case "QuitarAsistente":
                    QuitarAsistente(AsistenteSeleccionado);
                    break;
            }
        }
        ...
        protected void btnLimpiarFiltros_OnClick(object sender, EventArgs e)
        {
            ...
            txtNombreCooperativa.Text = string.Empty;
        ...
        protected void btnAdjuntar_OnClick(object sender, EventArgs e)
        {
            //verificar si se selecciono un archivo
            if (FUAsistentes.PostedFile.FileName == "")
            {
                lblMensajeError.Text = "No Seleccionaste ningun Archivo.";
                divMensajeError.Visible = true;
            }
            else
            {    //verificar extension
                string extension = Path.GetExtension(FUAsistentes.PostedFile.FileName);
                switch (extension.ToLower())
                {
                    ...
                    break;
                        //no validas
                    default:
                    lblResultadoAdjuntar.Text = "documento para adjuntar no válido";
                        return;
                ...
                try
                {
                    archivo = Path.GetFileName(FUAsistentes.PostedFile.FileName);
                    //nombre del archivo
                    string carpeta_completa = Path.Combine(url, archivo);
                    FUAsistentes.PostedFile.SaveAs(carpeta_completa);
                    lblMensajeExito.Text = "Archivo subido correctamente.";
                    divMensajeExito.Visible = true;
                    btnAdjuntar.Enabled = false;
                    FUAsistentes.Enabled = false;
                }
                catch (Exception ex)
                {
                    lblResultadoAdjuntar.Text = "Error: " + ex.Message;
